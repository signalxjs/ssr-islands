...
- [ ] `repository`, `homepage`, `bugs` fields point at `signalxjs/ssr-islands`.
...
For the **first publish** the package must be authenticated normally with an npm token. Then on https://www.npmjs.com/package/@sigx/ssr-islands/access:

1. Settings → Trusted Publishers → Add a Trusted Publisher.
2. Provider: GitHub Actions.
3. Repository owner: `signalxjs`. Repository: `ssr-islands`. Workflow filename: `release.yml`.
